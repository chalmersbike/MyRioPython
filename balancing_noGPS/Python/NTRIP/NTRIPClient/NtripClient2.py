...
class NtripClient(object):
    ...
    def getGGAString(self):
        ...
        if self.verbose:
            print("$%s*%s\r\n" % (ggaString, checksum))
        return "$%s*%s\r\n" % (ggaString, checksum)
        # return "$GPGGA,145010.0,5700.00000000,N,01200.00000000,E,4,10,1.0,0.000 M,0.0,M,,*69\r\n"
        ...
        try:
            while reconnectTry<=maxReconnect:
                ...
                if error_indicator==0:
                    # sys.stderr.write("Connected to %s:%d"%(self.caster,self.port) + "\n")
                    print("Connected to %s:%d"%(self.caster,self.port) + "\n")
                    print("")
                    sleepTime = 1
                    connectTime=datetime.datetime.now()

                    # print("Getting SOURCETABLE")
                    # print(self.getSourcetable())
                    # print("")
                    ...
                    now = datetime.datetime.utcnow()
                    # print("GET MSM_GNSS HTTP/1.1\r\nUser-Agent: NTRIP PythonClient/1.0\r\nAuthorization: Basic ***REMOVED******REMOVED***=\r\n\r\n$GPGGA,122704.098,5741.676,N,01158.588,E,1,12,1.0,0.0,M,0.0,M,,*65\r\n")
                    # self.socket.sendall("GET MSM_GNSS HTTP/1.1\r\nUser-Agent: NTRIP PythonClient/1.0\r\nAuthorization: Basic ***REMOVED******REMOVED***=\r\n\r\n$GPGGA,122704.098,5741.676,N,01158.588,E,1,12,1.0,0.0,M,0.0,M,,*65\r\n")

                    # test_str = "GET /MSM_GNSS HTTP/1.1\r\nUser-Agent: NTRIP sNTRIP/2.12.00n\r\nAuthorization: Basic ***REMOVED******REMOVED***=\r\nNtrip-GGA:  %s\r\n" % (self.getGGAString())
                    test_str = "GET /MSM_GNSS HTTP/1.1\r\nUser-Agent: NTRIP sNTRIP/2.12.00n\r\nAuthorization: Basic ***REMOVED******REMOVED***=\r\n\r\n"
                    print(test_str)
                    self.socket.sendall(test_str.encode())

                    while not found_header:
                        casterResponse=self.socket.recv(4096) #All the data
                        header_lines = casterResponse.split("\r\n".encode())
                        try:
                            print("casterResponse : " + casterResponse.decode(errors='backslashreplace'))
                        except:
                        ...
                        for line in header_lines:
                            if line=="":
                                ...
                                if self.verbose:
                                    try:
                                        # sys.stderr.write("Header: " + line.decode(errors='backslashreplace')+"\n")
                                        print("Header: " + line.decode(errors='backslashreplace')+"\n")
                                    except:
                            ...
                            if self.headerOutput:
                                try:
                                    self.headerFile.write(line.decode(errors='backslashreplace')+"\n")
                                except:
                                    self.headerFile.write(line)
                                    self.headerFile.write("\n")

                        for line in header_lines:
                            if line.find("SOURCETABLE".encode())>=0:
                                sys.stderr.write("Mount point does not exist")
                                sys.exit(1)
                            elif line.find("401 Unauthorized".encode())>=0:
                                sys.stderr.write("Unauthorized request\n")
                                sys.exit(1)
                            elif line.find("404 Not Found".encode())>=0:
                                sys.stderr.write("Mount Point does not exist\n")
                                sys.exit(2)
                            elif line.find("ICY 200 OK".encode())>=0:
                                #Request was valid
                                sys.stderr.write("RECEIVED \"ICY 200 OK\", sending GGA string ...\n")
                                self.socket.sendall(self.getGGAString().encode())
                            elif line.find("HTTP/1.0 200 OK".encode())>=0:
                                #Request was valid
                                sys.stderr.write("RECEIVED \"HTTP/1.0 200 OK\", sending GGA string ...\n")
                                self.socket.sendall(self.getGGAString().encode())
                            elif line.find("HTTP/1.1 200 OK".encode())>=0:
                                #Request was valid
                                sys.stderr.write("RECEIVED \"HTTP/1.1 200 OK\", sending GGA string ...\n")
                                self.socket.sendall(self.getGGAString().encode())
                    print("Sent GGA String, reading correction data ...")
...
if __name__ == '__main__':
    ...
    parser.add_option("-t", "--latitude", type="float", dest="lat", default=57.00, help="Your latitude.  Default: %default")
    parser.add_option("-g", "--longitude", type="float", dest="lon", default=12.00, help="Your longitude.  Default: %default")
    parser.add_option("-e", "--height", type="float", dest="height", default=100, help="Your ellipsoid height.  Default: %default")
