...
class Controller(object):
    ...
    def run(self):
        ...
        if gps_use and self.simulate_file == '':
            while self.bike.gps.instr.bytes_in_buffer > 100:
                self.bike.gps.instr.flush(pyvisa.constants.VI_READ_BUF_DISCARD)
        ...
        while self.controller_active or not self.gainingSpeedOver_flag:
            ...
            try:
                ...
                if gps_use:
                    ...
                    self.gps_read_quotient = ((time.time() - self.time_run_start) ) // (1.0 / gps_dataUpdateRate)   # The Quotient in GPS time
                    # print('GPS quotient: \n')
    ...
    def get_states(self):
        ...
        self.X_est, self.P_est = self.bike.Klm_Estimator.estimate(self)
        # print(self.X_est[2][0],self.X_est[1][0],self.X_est[0][0])
        self.x_estimated = self.X_est[0][0]
        self.y_estimated = self.X_est[1][0]
        self.velocity = self.X_est[2][0]
        self.heading_estimated = self.X_est[3][0]
        self.roll = self.X_est[4][0]
        self.steeringAngle = self.X_est[5][0]
        self.rollRate = self.X_est[6][0]
        self.steering_offset = self.X_est[7][0]
