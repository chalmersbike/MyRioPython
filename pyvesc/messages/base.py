...
class VESCMessage(type):
    ...
    def __init__(cls, name, bases, clsdict):
        ...
        for klass in bases:
            if isinstance(klass, VESCMessage):
                raise TypeError(u"VESC messages cannot be inherited.")
        # check for duplicate id
        if msg_id in VESCMessage._msg_registry:
            raise TypeError(u"ID conflict with %s" % unicode(VESCMessage._msg_registry[msg_id]))
        else:
        ...
        cls._field_names = []
        cls._field_scalars = []
        for field, idx in izip(cls.fields, xrange(0, len(cls.fields))):
        ...
        string_field_count = cls._fmt_fields.count(u's')
        if string_field_count > 1:
            raise TypeError(u"Max number of string fields is 1.")
        if u'p' in cls._fmt_fields:
            raise TypeError(u"Field with format character 'p' detected. For string field use 's'.")
        super(VESCMessage, cls).__init__(name, bases, clsdict)

    def __call__(cls, *args, **kwargs):
        instance = super(VESCMessage, cls).__call__()
        if u'can_id' in kwargs:
            cls.can_id = kwargs[u'can_id']
        else:
            cls.can_id = None
        if args:
            if len(args) != len(cls.fields):
                raise AttributeError(u"Expected %u arguments, received %u" % (len(cls.fields), len(args)))
            for name, value in izip(cls._field_names, args):
    ...
    def unpack(msg_bytes):
        ...
        if not (msg_type._string_field is None):
            # string field
            fmt_wo_string = msg_type._fmt_fields.replace(u'%u', u'')
            fmt_wo_string = fmt_wo_string.replace(u's', u'')
        ...
        if not (msg_type._string_field is None):
            ...
            setattr(msg,
                    string_field_name,
                    getattr(msg, string_field_name).decode(u'ascii'))
        return msg
    ...
    def pack(instance, header_only = None):
        ...
        if not instance._field_scalars:
            ...
            for field_name, field_scalar in izip(instance._field_names, instance._field_scalars):
        ...
        if not (instance._string_field is None):
            ...
            field_values[instance._string_field] = field_values[instance._string_field].encode(u'ascii')
