...
class Controller(object):
    ...
    def keep_the_bike_stable(self, velocity, references, states):
        ...
        self.pid_balance_control_signal = self.pid_balance.update(states[2]) # deltadot = PID(phidot)
        print 'pid = %f' % (self.pid_balance_control_signal)
    ...
    def variable_init(self):
        ...
        #numtaps = 10
        #cutoff = 1
        #self.b = firwin(numtaps, cutoff, nyq=CONTROLLER_FREQUENCY)
        #self.zpot = lfilter_zi(self.b, 1) * 0
        #self.zpotdot = lfilter_zi(self.b, 1) * 0
    ...
    def log_headerline(self):
        ...
        self.writer.writerow(
            ('Time', 'Calculation Time',  'Measured Velocity', 'Phi', 'Delta', 'Phi Dot',
             'Control Input',
             'a_x',  'ay', 'az', 'x','y','psi','nu','phi_ref','delta_ref','delta_ctrl_ref','x1_ref','x2_ref','x1','x2','delta_k','pot'))
             # ,'GPS_timestamp','GPS_x','GPS_y','latitude','longitude'))
             #,'laserranger'))  # headers
    ...
    def log_regular(self):
        ...
        self.writer.writerow(
            (
                ...
                "{0:.5f}".format(self.lqr_balance_control_signal),
        ...
        print 'Time=%f\tCalc Time = %f\t Potentio = %g \t Vel = %f\tphi = %f\tphi_ardu = %f\tdelta = %f\tx = %f \ty = %f\tpsi = %f\tnu = %f\t ' % (
            self.time_count, self.calculation_time, self.pot, self.velocity, 57.29577 * self.states[0],
            57.29577 * self.extra_data[0], 57.29577 * self.states[1],self.x,self.y,self.psi,self.nu)
    ...
    def keep_the_bike_stable_2states(self, velocity, references, states):
        # # feedback_gains = self.get_2state_feedback_gains(velocity)
        # # feedback_gains = [7.6618,   21.1350]
        # # feedback_gains = [10.8103, 42.9589] #[10 -3 -3 1] 1
        # feedback_gains = [8.9004 /2.0, 26.8189/2.0] #[10 -9 -9 10] 1
        # self.AngRef = sum([(r - x) * k for r, x, k in zip(
        #     references, states, feedback_gains
        # )])
        # self.lqr_balance_control_signal = self.pos2vel(self.AngRef)
        ...
        self.pid_balance_control_signal = self.pid_balance.update(states[2]) # deltadot = PID(phidot)
        self.pid_balance_control_signal = 1*self.pid_balance_control_signal
        #print 'pid = %f' % (self.pid_balance_control_signal)

        #Implementing the potentiometer
        #self.pot = 0
        #self.pot = ((self.bike.potent.read_pot_value() / 0.29) * 5 - 2.5) * deg2rad * 2
        #self.pot = round(self.pot,2)
        #self.pot_filt, self.zpot = lfilter(self.b, 1, [self.pot], zi=self.zpot)
        #self.pot_filt = self.pot_filt[0]
        #self.potdot = (self.pot_filt-self.pot_previous)/sample_time
        #self.potdot_filt, self.zpotdot = lfilter(self.b, 1, [self.potdot], zi=self.zpotdot)
        #print 'phidot = %f ; pot = %f ; pot_filt = %f ; potdot = %f ; potdot_filt = %f ; pid = %f ; pid+potdot_filt = %f' % (states[2],self.pot,self.pot_filt,self.potdot,self.potdot_filt,self.pid_balance_control_signal,self.pid_balance_control_signal+self.potdot_filt)
        #self.potdot = self.potdot_filt[0]
        #self.pot_previous = self.pot_filt
        #self.potdot = min(max(self.potdot,-0.07),0.07)

        # self.pid_balance_control_signal = self.pid_balance_control_signal + self.pot
        #self.pid_balance_control_signal = self.pid_balance_control_signal + self.potdot
