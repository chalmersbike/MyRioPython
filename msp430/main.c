...
//         --|RST                |
//           |                   |
//           |         P1.2/TA0.1|--> CCR1 - PWM (Maxon 50/5 Steering Motor Controller)
//     RX<-- |P3.4     P1.3/TA0.2|--> CCR2 - PWM (Jaguar Brake Actuator)
//     TX<-- |P3.3     P1.4/TA0.3|--> CCR3 - PWM (Maxon 50/4 Longitudinal Motor Controller)
//
//  JAGUAR
...
//
//  Maxon 50/4 Longitudinal Motor Controller
//  TA0CCR = 1050; // corresponds to 1.0ms (maxon neutral)
//  TA0CCR = 2100; // corresponds to 2.0ms (maxon full forward)
//
//  MAXON 50/5 Steering Motor Controller
...
#define PWM_MIN_JAGUAR (710)       // 0.67 ms
#define PWM_NEUTRAL_JAGUAR (1580)  // 1.5 ms
#define PWM_MAX_JAGUAR (2450)      // 2.33 ms

#define PWM_NEUTRAL_SHIMANO (1050) // 1.0 ms
#define PWM_MAX_SHIMANO (2100)     // 2.0 ms

#define PWM_MIN_MAXON (1050)       // 1.0 ms
#define PWM_NEUTRAL_MAXON (1580)   // 1.5 ms
#define PWM_MAX_MAXON (2100)       // 2.0 ms
...
void process_char(char input){
    ...
    // Maxon Steering Motor
        ...
        long int duty = PWM_NEUTRAL_MAXON;
        if(uart_buffer[0] != '\0'){
            ...
            if(endptr != &uart_buffer[4]){
                ...
                duty = PWM_NEUTRAL_MAXON;
            }

            if(duty > PWM_MAX_MAXON){
                duty = PWM_MAX_MAXON;
            }
            else if(duty < PWM_MIN_MAXON){
                duty = PWM_MIN_MAXON;
            }
    ...
    // Brake Actuator
        ...
        if(uart_buffer[0] != '\0'){
            ...
            if(duty > PWM_MAX_JAGUAR){
                duty = PWM_MAX_JAGUAR;
            }
            else if(duty < PWM_MIN_JAGUAR){
                duty = PWM_MIN_JAGUAR;
            }
    ...
    // Drive Motor
        ...
        if(uart_buffer[0] != '\0'){
            ...
            if(duty > PWM_MAX_SHIMANO){
                duty = PWM_MAX_SHIMANO;
            }
            else if(duty < PWM_NEUTRAL_SHIMANO){
                duty = PWM_NEUTRAL_SHIMANO;
            }
