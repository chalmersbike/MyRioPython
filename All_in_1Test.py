#!/usr/bin/env python
from sensors import Encoder, HallSensor, IMU,  GPS #SafetyStop,
...
class Test(object):
    def __init__(self):
        ...
        # Hall sensor
        number_samples_HallSensor = raw_input('Input the number of samples or press ENTER for 20 samples for the Hall sensor test, rotate the rear wheel for the reading! ')
        ...
        for x in range(1,int(number_samples_HallSensor)+1):
            print 'Time=%f\t Vel = %f\n km/h' % (time.time()-start_time,self.hall_sensor.get_velocity())
            time.sleep(0.5)

        # Encoder
        number_samples_Encoder = raw_input('Input the number of samples or press ENTER for 20 samples for the encoder test, rotate the handle bar for the reading! ')
        ...
        # IMU
        number_samples_IMU = raw_input('Input the number of samples or press ENTER for 20 samples for the IMU test, move the bike body for the reading! ')
        self.a_imu = IMU()
        start_time = time.time()
        ...
        # GPS
        #number_samples_GPS = raw_input('Input the number of samples or press ENTER for 20 samples for the GPS test, move the GPS for the reading! ')
        #gps = GPS()
        #start_time = time.time()
        ...
        if not input1:
            ...
            if not input1:
                ...
                print 'Now all tests have been done, except the steering motor, please do it carefully by running the easy_start and disconnect the rear motor'
            else:
                print 'Test Finished'
        else:
