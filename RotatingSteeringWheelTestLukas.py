...
class Test(object):
    def __init__(self):
        ...
        if not input1:
            ...
            raw_input('The test will start now and last 20 seconds! ')
            ...
            time_now = time.time()
            while time_now - start_time < 20:
                time_now = time.time()
                print 'Time=%f\t tdelta = %f\t Angular Velocity = %f\t' % (
                time_now - start_time, -57.29577 * self.encoder.get_angle(), angular_velocity)
                # Write to CSV file
                writer_steering_motor.writerow((time.time() - start_time, -57.29577 *self.encoder.get_angle(), angular_velocity))
                time.sleep(0.2)
