...
class Controller(object):
    ...
    def get_states(self):
        ...
        # delta_state = self.bike.get_handlebar_angle() + self.potential
        ...
        phi_d_state = phi_d_state + self.potential
    ...
    def log_headerline(self):
        ...
        self.writer.writerow(
            ('Time', 'Calculation Time', 'Measured Velocity', 'Phi', 'Delta', 'Phi Dot',
             'Control Input',
             'a_x', 'ay', 'az', 'x', 'y', 'psi', 'nu', 'phi_ref', 'delta_ref', 'delta_ctrl_ref', 'x1_ref', 'x2_ref',
             'x1', 'x2', 'delta_k', 'potential'))
        # ,'GPS_timestamp','GPS_x','GPS_y','latitude','longitude'))
        # ,'laserranger'))  # headers
    ...
    def state_calculate(self):
        ...
        self.xk = np.linalg.inv(O_bike) * (
                    np.array([[phi], [Phi_dot]]) - np.vstack((D_bike, D_bike + C_bike.dot(B_bike))) * delta)
    ...
    def keep_the_bike_stable_2states(self, velocity, references, states):
        ...
        print 'phidot(deg/s) = ' + str(57.29577 * states[2]) + ' ; pid_signal(deg/s) = ' + str(
            57.29577 * self.pid_balance_control_signal)
